...
function RetornarInformacoesCao($Id)
{
	...
	{
	require("../Funcoes/Conexao.php");
	$query = "Select e.NoQualificacaoCao, a.NuRegistroNacional, b.NoSelecao, c.NoRaioX, d.NoAdestramento, a.NoTatuagem, a.DaSelecao, a.TPSexo, a.DsAdestramento From (((TBCachorro as a left join TBSelecao as b on a.IdSelecao = b.IdSelecao) Left Join TBRaioX as c on a.IdRaioX = c.IdRaioX) left join TBAdestramento as d on a.IDAdestramento = d.IDAdestramento) left join TBQualificacaoCao as e On a.IdQualificacaoCao = e.IdQualificacaoCao Where a.IDCachorro = $Id";
	...
	while ($row = mysql_fetch_array($result))
	{
		...
		*/
		...
		if ($row[DsAdestramento] != ""){
			$Retorno = $Retorno . "  $row[DsAdestramento]   ";
		}
		
		$Retorno = $Retorno . verRaioX($Id);
...
function verRaioX($animal){
	...
	$sql = "
		select 
			...
			e.NoRaioX as rx4
		from 
			(((tbcachorro as a left join tbraiox as b on a.idraiox1 = b.idraiox)
			left join tbraiox as c on a.idraiox2 = c.idraiox)
			left join tbraiox as d on a.idraiox3 = d.idraiox)
			left join tbraiox as e on a.idraiox4 = e.idraiox
	...
	$result = mysql_query($sql) or die("Erro7: " . $query);
	while ($row = mysql_fetch_array($result))
	{
		if ($row[rx1] != "")
			$Retorno .= "  $row[rx1]   ";
